...
$category = 'internet profesional';
$f_scripts = "<script>fillCaptionText(['".Tools::getLocale()->getString('INDEX_BREADCRUMP')."','".Tools::getLocale()->getString('PROFESSIONAL_BREADCRUMP')."']); changeCaptionImg('"._IMAGE_PATH_."caption/bg1.jpg');actUserBar();</script>";
...
foreach($services as $service){
	...
	if($res = $sql->fetch_array() and $x < $limit_preview){
		...
		$preview.= Template::getPreviewTable(array(
			...
			'<big><big><strong>'.Tools::num_format($res['service_fee'],2).'€</strong></big></big>'),'<a href="add-to-cart/'.$res['id_service'].'"><div class="glyphicon-pro cart-preview"></div></a>');
	}
	$x++;
}
...
$content.= Template::getPreviewTable(array(
	...
	'<big><big><strong>'.Tools::num_format($tplana['fee'],2).'€</strong></big></big><span class="min-60">/'.Tools::getLocale()->getString('mes').'</span><br /><span class="min-60">'.Tools::num_format($tplana['attr1']).' '.Tools::getLocale()->getString('min').' '.Tools::getLocale()->getString('to').' '.Tools::getLocale()->getString('landline').' '.Tools::getLocale()->getString('nationals').'<br />'.Tools::num_format($tplana['attr2']).' '.Tools::getLocale()->getString('min').' '.Tools::getLocale()->getString('to').' '.Tools::getLocale()->getString('smartphone').' '.Tools::getLocale()->getString('nationals').'</span>'),'<a href="'._ROOT_PATH_.'add-sub-to-cart/'.$tplana['id'].'"><div class="glyphicon-pro cart-preview"></div></a>');

$tpl = new Template($f_scripts);
$tpl->setContentPage('tpl.php',array(
	'[TITLE]' => Tools::getLocale()->getString('PROFESSIONAL_TITLE'),
	'[SUBTITLE]' => Tools::getLocale()->getString('PROFESSIONAL_SUBTITLE'),
	'[PREVIEW]' => $preview,
	'[CONTENT]' => $content,
	'[FOOTER]' => Tools::getLocale()->getString('PROFESSIONAL_FOOTER')
	)
);
